 ...
 *   1. **API key** — Bearer token with `blq_` prefix (or legacy `abc_` prefix), hashed & looked up in PostgreSQL
 ...
 *   - API keys start with `blq_` (current) or `abc_` (legacy, still accepted)
...
import { isApiKey } from './api-key-utils.ts';
...
/** Check whether a Bearer token looks like an API key (`blq_` prefix, or legacy `abc_` prefix). */
 ...
 *    starts with `blq_` (or legacy `abc_`) it is hashed and looked up in PostgreSQL via
...
export async function authenticateRequestUnified(
    ...
    return {
        context: { ...ANONYMOUS_AUTH_CONTEXT },
        response: new Response(
            JSON.stringify({ success: false, error: 'Unrecognised credential format — expected a blq_-prefixed API key (or legacy abc_-prefixed key)' }),
            { status: 401, headers: { 'Content-Type': 'application/json' } },
        ),
    };
}
