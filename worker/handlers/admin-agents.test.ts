...
Deno.test('handleAdminListAgentSessions — 403 for non-admin user', async () => {
    ...
    const res = await handleAdminListAgentSessions(makeAppContext(req, makeEnv(), makeUserContext()));
    assertEquals(res.status, 403);
    const body = await res.json() as Record<string, unknown>;
    assertEquals(body.success, false);
});

Deno.test('handleAdminListAgentSessions — 503 when prisma not in context', async () => {
    const req = new Request('http://localhost/admin/agents/sessions');
    const res = await handleAdminListAgentSessions(makeAppContext(req, makeEnv(), makeAdminContext()));
...
Deno.test('handleAdminListAgentSessions — 400 on invalid pagination params', async () => {
    ...
    const res = await handleAdminListAgentSessions(makeAppContext(req, makeEnv(), makeAdminContext(), prisma));
...
Deno.test('handleAdminGetAgentSession — 403 for non-admin user', async () => {
    ...
    const res = await handleAdminGetAgentSession(makeAppContext(req, makeEnv(), makeUserContext()), VALID_UUID);
    assertEquals(res.status, 403);
});
...
Deno.test('handleAdminGetAgentSession — 400 on invalid UUID', async () => {
    ...
    const res = await handleAdminGetAgentSession(makeAppContext(req, makeEnv(), makeAdminContext(), makeMockPrisma()), 'not-a-uuid');
    assertEquals(res.status, 400);
    const body = await res.json() as Record<string, unknown>;
    assertEquals(body.success, false);
});

Deno.test('handleAdminGetAgentSession — 503 when prisma not in context', async () => {
    const req = new Request(`http://localhost/admin/agents/sessions/${VALID_UUID}`);
    const res = await handleAdminGetAgentSession(makeAppContext(req, makeEnv(), makeAdminContext()), VALID_UUID);
    assertEquals(res.status, 503);
});
...
Deno.test('handleAdminGetAgentSession — 404 when session not found', async () => {
    const prisma = makeMockPrisma();
    const req = new Request(`http://localhost/admin/agents/sessions/${VALID_UUID}`);
    const res = await handleAdminGetAgentSession(makeAppContext(req, makeEnv(), makeAdminContext(), prisma), VALID_UUID);
...
Deno.test('handleAdminListAgentAuditLog — 403 for non-admin user', async () => {
    ...
    const res = await handleAdminListAgentAuditLog(makeAppContext(req, makeEnv(), makeUserContext()));
    assertEquals(res.status, 403);
});

Deno.test('handleAdminListAgentAuditLog — 503 when prisma not in context', async () => {
    const req = new Request('http://localhost/admin/agents/audit');
    const res = await handleAdminListAgentAuditLog(makeAppContext(req, makeEnv(), makeAdminContext()));
    assertEquals(res.status, 503);
});
...
Deno.test('handleAdminListAgentAuditLog — 200 returns audit entries', async () => {
    ...
    const req = new Request('http://localhost/admin/agents/audit');
    const res = await handleAdminListAgentAuditLog(makeAppContext(req, makeEnv(), makeAdminContext(), prisma));
    assertEquals(res.status, 200);
    const body = await res.json() as Record<string, unknown>;
    assertEquals(body.success, true);
    assertEquals((body as Record<string, unknown>).total, 1);
...
Deno.test('handleAdminTerminateAgentSession — 400 on invalid UUID', async () => {
    ...
    const res = await handleAdminTerminateAgentSession(makeAppContext(req, makeEnv(), makeAdminContext(), makeMockPrisma()), 'bad-id');
    assertEquals(res.status, 400);
    const body = await res.json() as Record<string, unknown>;
    assertEquals(body.success, false);
});

Deno.test('handleAdminTerminateAgentSession — 503 when prisma not in context', async () => {
    const req = new Request(`http://localhost/admin/agents/sessions/${VALID_UUID}`, { method: 'DELETE' });
    const res = await handleAdminTerminateAgentSession(makeAppContext(req, makeEnv(), makeAdminContext()), VALID_UUID);
    assertEquals(res.status, 503);
});
...
Deno.test('handleAdminTerminateAgentSession — 404 when session not found', async () => {
    const prisma = makeMockPrisma();
    const req = new Request(`http://localhost/admin/agents/sessions/${VALID_UUID}`, { method: 'DELETE' });
    const res = await handleAdminTerminateAgentSession(makeAppContext(req, makeEnv(), makeAdminContext(), prisma), VALID_UUID);
    assertEquals(res.status, 404);
});
