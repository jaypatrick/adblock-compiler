 ...
 *   - 200 with provider='mailchannels' when FROM_EMAIL set and no binding or queue
 ...
 *   - 403 when user is not an admin
 *   - 503 when no provider is configured (no EMAIL_QUEUE, SEND_EMAIL, or FROM_EMAIL)
...
Deno.test('handleAdminEmailConfig — provider=mailchannels when FROM_EMAIL set and no binding or queue', async () => {
    const req = new Request('http://localhost/admin/email/config');
