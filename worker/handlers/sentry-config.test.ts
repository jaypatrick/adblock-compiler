 ...
 *
 * Covers:
 *  - Returns { dsn: null } when SENTRY_DSN is unset
...
Deno.test('handleSentryConfig — returns { dsn: null } when SENTRY_DSN is unset', async () => {
    const env = makeEnv(); // no SENTRY_DSN
    const response = handleSentryConfig(env);
    const body = await response.json() as { dsn: string | null };
