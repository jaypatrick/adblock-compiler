...
This directory contains a **Starlight (Astro)** documentation site for Bloqr — a modern alternative to the current [mdBook](../book.toml) setup.

Both sites coexist independently. The existing mdBook pipeline (`book.toml`, `wrangler.docs.toml`, `docs.bloqr.dev`) is **not modified** by this scaffold.
...
CI auto-deploys on push to `main` when files under `docs/**` or `docs-starlight/**` change (see [`.github/workflows/docs-starlight.yml`](../.github/workflows/docs-starlight.yml)).
...
Content is read directly from `../docs/` via the `srcDir: '../docs'` setting in `astro.config.ts`. **No Markdown files are copied or duplicated.**
...
`@beoe/rehype-mermaid` is listed as a dev dependency. To activate it, add it to the `rehypePlugins` array in `astro.config.ts`:
...
Some Mermaid fences in the existing docs use `mermaid` as the language identifier — verify they render correctly after wiring.

### 2. OpenAPI plugin config
`starlight-openapi` is installed but not yet configured. To add an OpenAPI reference panel:
...
### 3. Logo asset
`src/assets/logo.svg` is a minimal placeholder shield icon. Replace with the official Bloqr SVG logo.

### 4. Fonts
`custom.css` loads Inter and JetBrains Mono from Google Fonts CDN. For fully offline/privacy-respecting builds, replace with `@fontsource/inter` and `@fontsource/jetbrains-mono` npm packages and import them in `custom.css`.

### 5. `srcDir` and frontmatter
Starlight expects frontmatter in Markdown files. Existing `docs/**/*.md` files without frontmatter will still render, but pages with `title:` in frontmatter will override the sidebar label. Add frontmatter gradually as needed.
