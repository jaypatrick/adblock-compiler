...
async function stepGenerateCloudflareSchema(dryRun: boolean): Promise<void> {
    ...
    if (patchedOps.length > 0) {
        console.log(`\n⚠️  Patched ${patchedOps.length} operation(s) with stub 2xx response (fix upstream openapi.yaml):`);
...
async function stepGeneratePostmanCollection(dryRun: boolean): Promise<void> {
    ...
    if (dryRun) {
        console.log(`🔍 Dry-run: would write ${COLLECTION_OUTPUT_PATH} (${requestCount} requests) and ${ENVIRONMENT_OUTPUT_PATH}`);
        return;
    }
