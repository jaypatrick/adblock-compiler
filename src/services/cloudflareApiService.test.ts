...
Deno.test('CloudflareApiService - listApiShieldSchemas', async (t) => {
    await t.step('should return schema list from client', async () => {
        ...
        assertEquals((schemas[0] as OldPublicSchema).schema_id, 'schema-id-1');
        assertEquals((schemas[0] as OldPublicSchema).validation_enabled, true);
    });
...
Deno.test('CloudflareApiService - uploadApiShieldSchema', async (t) => {
    await t.step('should return create response from client', async () => {
        ...
        assertEquals((result as UserSchemaCreateResponse).schema.schema_id, 'schema-id-new');
    });
                ...
                userSchemas: {
                    ...
                    create: (params: unknown) => {
                        ...
                        return Promise.resolve({
                            schema: { schema_id: 'new-id', name: 'n', kind: 'openapi_v3', created_at: '' } as OldPublicSchema,
                        } as UserSchemaCreateResponse);
                    },
                },
            },
...
Deno.test('CloudflareApiService - enableApiShieldSchema', async (t) => {
    await t.step('should return updated schema from client', async () => {
        ...
        assertEquals((result as OldPublicSchema).schema_id, 'schema-id-1');
        assertEquals((result as OldPublicSchema).validation_enabled, true);
    });
