...
| Angular frontend | Vitest + `@analogjs/vitest-angular` | `frontend/` | `pnpm --filter bloqr-frontend run test` |
| E2E (Worker) | Deno native | `worker/*.e2e.test.ts` | included in `test:worker` |
| E2E (Frontend) | Playwright | `frontend/e2e/` | `pnpm --filter bloqr-frontend run test:e2e` |
...
2. **Frontend Vitest** — `pnpm --filter bloqr-frontend run test`
