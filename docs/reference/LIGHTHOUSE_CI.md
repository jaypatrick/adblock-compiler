...
|---|---|---|
| `deno task lighthouse:install` | `pnpm add --global @lhci/cli` | Install `@lhci/cli` globally via pnpm |
| `deno task lighthouse:run` | `pnpm exec lhci autorun` | Run `lhci autorun` using `.lighthouserc.json` |
| `deno task lighthouse:summary` | `deno run ... scripts/lighthouse-summary.ts` | Generate Markdown score table from `lhci-results/` |
