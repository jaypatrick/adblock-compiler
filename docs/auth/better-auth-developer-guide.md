...
| `sentinel({ apiKey, kvUrl, security })` | `@better-auth/infra` | — | Credential stuffing protection, impossible travel detection, bot/IP blocking, device notifications; requires `BETTER_AUTH_API_KEY` passed explicitly via `apiKey: env.BETTER_AUTH_API_KEY` (same reason as `dash()`) |
