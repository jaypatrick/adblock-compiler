...
if missing:
    ...
    print(f"     # or add missing packages: uv add --directory tools {' '.join(missing)}")
    sys.exit(1)
...
def _record(name: str, status: str, detail: str, data: dict | None = None) -> None:
    ...
    _default_icon = "\u2022"
    console.print(f"  {icons.get(status, _default_icon)} [{c}]{name}[/{c}]: {detail}")
...
def wrangler(*args: str, timeout: int = 30) -> tuple[bool, str, str]:
    ...
    try:
        ...
        stderr_only_benign = all(
            any(pat in line for pat in _BENIGN) or not line.strip() for line in stderr.splitlines()
        )
...
def _verify_email_in_neon(user_id: str) -> bool:
    ...
    if not neon_url:
        console.print(
            "[yellow]\u26a0\ufe0f  NEON_URL not set \u2014 cannot pre-verify email, sign-in will likely fail[/yellow]"
        )
        return False
    try:
        ...
        if not user_tbl:
            console.print(
                f"[yellow]\u26a0\ufe0f  Cannot pre-verify: no user table found (discovered: {list(tbl_map.values())})[/yellow]"
            )
            cur.close()
            conn.close()
            return False
        ...
        verified = False
        for val in (datetime.now(UTC), True):
            try:
                cur.execute(
...
def check_signin(signup_result: dict | None, signup_succeeded: bool = False) -> dict | None:
    ...
    try:
        ...
        (ok if token else fail)(
            "session.token present", f"{token[:24]}\u2026" if token else f"missing \u2014 keys: {list(d.keys())}", d
        )
        (ok if user.get("id") else fail)(
            "user object",
            f"id={user.get('id')}  email={user.get('email')}  tier={user.get('tier', '?')}  role={user.get('role', '?')}"
            if user.get("id")
            else f"no user.id \u2014 keys: {list(user.keys())}",
            d,
        )
        (ok if session.get("id") else fail)(
            "session object",
            f"id={str(session.get('id', ''))[:20]}\u2026  expires={session.get('expiresAt', '?')}"
            if session.get("id")
            else f"no session.id \u2014 keys: {list(session.keys())}",
            d,
        )
        return d
    except Exception as e:
        ...
        if session_id:
            try:
                ...
                if sess:
                    ...
                    warn(
                        "Session in Neon",
                        "Not in Postgres \u2014 likely stored in KV only (expected when KV is bound).",
                    )
            except Exception as e:
...
def summarise_tail(lines: list[str]) -> None:
    ...
    (ok if not error_logs else warn)(
        "Worker error logs", f"{len(error_logs)} line(s)" if error_logs else "none", {"errors": error_logs[:10]}
    )
    (ok if auth_logs else warn)(
        "Auth log events", f"{len(auth_logs)} line(s)" if auth_logs else "none captured", {"events": auth_logs[:20]}
    )
...
def cleanup_test_user_d1(binding: str, label: str) -> bool:
    ...
    success, stdout, stderr = wrangler(
        ...
        "--command",
        f"DELETE FROM user WHERE email = '{escaped_email}';",
        timeout=60,
    )
    if success:
...
def cleanup_all_healthcheck_data() -> None:
    ...
    try:
        ...
        if not neon_url:
            ...
            try:
                ...
                if not user_tbl:
                    console.print(
                        f"[yellow]\u26a0\ufe0f  No user table found \u2014 tables: {list(tbl_map.values())}[/yellow]"
                    )
                else:
                    if not re.match(_SAFE_TABLE_NAME_RE, user_tbl):
                        ...
                        cur.execute(
                            psycopg2.sql.SQL("SELECT id FROM {} WHERE email LIKE 'healthcheck-%'").format(
                                psycopg2.sql.Identifier(user_tbl)
                            )
                        )
                        ...
                        for orphan_concept in ("session", "account"):
                            ...
                            try:
                                cur.execute(
                                    psycopg2.sql.SQL("DELETE FROM {} WHERE {} = ANY(%s)").format(
                        ...
                        if verif_tbl:
                            if not re.match(_SAFE_TABLE_NAME_RE, verif_tbl):
                                ...
                                try:
                                    cur.execute(
                                        psycopg2.sql.SQL("DELETE FROM {} WHERE {} LIKE 'healthcheck-%'").format(
                        ...
                        cur.execute(
                            psycopg2.sql.SQL("DELETE FROM {} WHERE email LIKE 'healthcheck-%'").format(
                                psycopg2.sql.Identifier(user_tbl)
                            )
                        )
                        neon_rows += cur.rowcount
                        console.print(f'[dim]  \u00b7 Deleted {cur.rowcount} user row(s) from "{user_tbl}"[/dim]')
            finally:
                cur.close()
                conn.close()
    except Exception as e:
    ...
    try:
        ...
        success, stdout, stderr = wrangler(
            ...
            "--json",
            "--command",
            "DELETE FROM user WHERE email LIKE 'healthcheck-%';",
            timeout=60,
        )
        if success:
            raw = _extract_json(stdout)
            if raw:
                try:
                    ...
                    console.print(f"[yellow]\u26a0\ufe0f  D1 {binding}: could not parse result JSON: {e_json}[/yellow]")
            else:
            ...
            console.print(f"[yellow]\u26a0\ufe0f  D1 {binding} cleanup failed: {(stderr or stdout)[:300]}[/yellow]")
    except Exception as e:
    ...
    try:
        ...
        if not success:
            console.print(f"[yellow]\u26a0\ufe0f  KV list failed: {(stderr or stdout)[:300]}[/yellow]")
        else:
            raw = _extract_json(stdout)
            if raw:
                keys = json.loads(raw)
                if isinstance(keys, list):
                    hc_key_names = [
                        k.get("name", "") for k in keys if isinstance(k, dict) and "healthcheck-" in k.get("name", "")
                    ]
                    for key_name in hc_key_names:
                        del_ok, _, _ = wrangler("kv", "key", "delete", "--binding", kv_binding, key_name)
                        if del_ok:
                            kv_keys += 1
    ...
    try:
        ...
        if not api_key:
            ...
            if r.ok:
                ...
                console.print(f"[yellow]\u26a0\ufe0f  Admin API list-users failed: HTTP {r.status_code}[/yellow]")
    except Exception as e:
